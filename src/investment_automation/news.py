...
POSITIVE_WORDS = ("approved", "launch", "listing", "partnership", "buy", "bull", "surge", "record", "win")
NEGATIVE_WORDS = ("hack", "exploit", "lawsuit", "charged", "ban", "crash", "stolen", "fraud", "probe")
...
class NewsClient:
    ...
    def fetch_events(self) -> list[NewsEvent]:
        ...
        if self.settings.kol_monitor_enabled:
            for profile_name, feed_url in self._kol_feed_entries():
                try:
                    events.extend(self._fetch_feed(feed_url, source=f"kol:{profile_name}", is_kol=True))
                except Exception:
    ...
    def _fetch_feed(self, feed_url: str, source: str = "", is_kol: bool = False) -> list[NewsEvent]:
        ...
        return [event for event in (self._parse_item(source, item, is_kol=is_kol) for item in items[:30]) if event is not None]

    def _parse_item(self, source: str, item: ET.Element, is_kol: bool = False) -> Optional[NewsEvent]:
        ...
        source_weight = self.settings.kol_source_weight if is_kol or source.startswith("kol:") else self._source_weight(source, url)
        freshness = max(0.0, 1.0 - max(fetched_ts - published_ts, 0) / max(self.settings.news_event_ttl_seconds, 1))
        confidence = min(1.0, 0.2 + 0.25 * len(topics) + 0.12 * len(entities) + 0.25 * source_weight + 0.18 * freshness)
        ttl = self.settings.kol_event_ttl_seconds if is_kol or source.startswith("kol:") else self.settings.news_event_ttl_seconds
    ...
    def _classify(self, text: str) -> tuple[list[str], list[str]]:
        lowered = text.lower()
        topics = [topic for topic, words in TOPIC_RULES.items() if any(self._contains_term(lowered, word) for word in words)]
    ...
    def _extract_token_candidates(self, text: str) -> list[str]:
        candidates = []
        candidates.extend(match.group(1).lower() for match in re.finditer(r"\$([a-zA-Z][a-zA-Z0-9_]{1,12})", text))
        candidates.extend(match.group(1).lower() for match in re.finditer(r"#([a-zA-Z][a-zA-Z0-9_]{2,20})", text))
        candidates.extend(match.group(1).lower() for match in re.finditer(r"\b([A-Z]{2,8})\b", text))
        return [item for item in candidates if item not in {"the", "and", "for", "you", "this", "that", "with", "from"}]
    ...
    def _parse_kol_profiles(self) -> dict[str, tuple[str, ...]]:
        ...
        for entry in self.settings.kol_profiles:
            ...
            profiles[name.strip().lower()] = tuple(item.strip().lower() for item in aliases.split("|") if item.strip())
        return profiles
...
def event_match_score(scan_text: str, event: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    hits = sorted({term for term in terms if term and re.search(rf"(^|[^a-z0-9]){re.escape(term)}([^a-z0-9]|$)", lowered)})
    if not hits:
        return 0.0, []
    ...
    return min(1.0, (0.35 + 0.25 * len(hits) + 0.25 * confidence + 0.15 * source_weight) * sentiment_multiplier), hits
